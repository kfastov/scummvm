...
* **Картинки.** В книге 2484 заголовка DIB (``BITMAPINFOHEADER``), из них 2320
  восьмибитных. Пиксели сжаты своим RLE: ``c <= 0xf5`` — серия длиной ``c + 3``,
  ``c > 0xf5`` — ``c - 0xf5`` литеральных байт. Строк поток не знает: серия
  спокойно переходит на следующую строку (ledger/0030).
