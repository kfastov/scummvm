...
//   * картинки описаны обычными DIB (BITMAPINFOHEADER + палитра). Там, где
//     перед заголовком стоит файловая шапка BM, пиксели лежат несжатыми и
//     разворачиваются штатно; у остальных после палитры идёт свой поток,
//     начинающийся с `01 00 00 00 01 00 00 00 00 00`. Стандартным RLE8 он не
//     разворачивается — проверено, — и пока не разобран;
...
struct Image {
	...
	uint32 pixels = 0;   ///< смещение пиксельных данных
	uint32 dataSize = 0; ///< размер пиксельных данных

	/// Правда, если перед заголовком стоит файловая шапка BM и данные лежат
	/// несжатыми. У остальных картинок книги после палитры идёт свой,
	/// неразобранный поток: он начинается с `01 00 00 00 01 00 00 00 00 00`,
	/// стандартным RLE8 не разворачивается, и рисовать его нельзя — получается
	/// шум. Такие картинки движок пропускает, а не показывает как попало.
	bool raw = false;
};
...
public:
	...
	Graphics::Surface *decodeImage(const Image &img, Graphics::Palette &palette) const;
