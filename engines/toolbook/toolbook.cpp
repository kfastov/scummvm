...
bool ToolBookEngine::runHandler(const Handler &handler,
	...
	// Наборы среды. Значение набора — четырёхбайтовая ссылка с тегом 0x40:
	// `ValueNewStack` (MTB40BAS seg118:0x0000) отводит заголовок с признаком
	// вида 0x10, а тег значения — это признак, умноженный на четыре (по нему
	// же разбирает `StackPush`, таблица seg118:0x07e6).
	auto newCollection = [&]() {
		Value v;
		v.collectionId = _nextCollection++;
		v.number = v.collectionId;
		v.type = 0x40;
		v.width = 4;
		_stackStore[v.collectionId].clear();
		return v;
	};
	auto collectionOf = [&](const Value &v) -> Common::Array<Value> * {
		if (!v.collectionId || !_stackStore.contains(v.collectionId))
			return nullptr;
		return &_stackStore[v.collectionId];
	};
	// `StackPush` (MTB40BAS.134): если элемент сам набор (тег 0x40..0x43), ветка
	// seg118:0x096c обходит его с последнего элемента к первому, каждый — через
	// `StackPushOne`. У нас `StackPushOne` — вставка в начало, поэтому весь
	// чужой набор оказывается впереди целиком и в своём порядке.
	auto stackPush = [&](Common::Array<Value> &into, const Value &element) {
		Common::Array<Value> *nested = collectionOf(element);
		if (!nested) {
			into.insert_at(0, element);
			return;
		}
		const Common::Array<Value> source = *nested;   // копия: набор бывает свой же
		for (int i = (int)source.size() - 1; i >= 0; i--)
			into.insert_at(0, source[i]);
	};
	// Число из значения так же, как его берёт арифметика: элемент набора мог
	// прийти строкой.
	auto numberOf = [&](const Value &v) {
		return v.isString ? (int32)atoi(v.string.c_str()) : (int32)v.number;
	};
	...
	for (uint steps = 0; ip < codeEnd && steps < 10000; steps++) {
		...
		case 0x20: {
			...
			if (id == 192) {
				// Сборка набора. Обёртка RUN67:0x0cfe читает операнды сверху
				// вниз как `[u16 признак][u16 счётчик][элементы по 4 байта]` —
				// отсюда и `cleanup = 2 + 2 + 4×счётчик`.
				//
				// Признак выбирает один из двух способов, и оба видны в самой
				// обёртке:
				//
				//   ≠0 — `ValueNewStack` нулевого размера, дальше каждый элемент
				//        через `StackPush` (MTB40BAS.134), то есть с разбором
				//        вложенных наборов;
				//   =0 — `ValueNewStack` на счётчик мест, дальше каждый элемент
				//        через `StackSetElement` (MTB40BAS.133) с индексом,
				//        который отсчитывается вниз от `счётчик−1` (`dec [bp+8]`
				//        на 0x0d37, а в вызов идёт уже уменьшенное значение).
				//
				// Способы выглядят противоположными, но дают один и тот же
				// порядок: у среды индекс 0 — это физически последний элемент
				// (`StackElement` берёт позицию `размер − 1 − индекс`). В обоих
				// случаях элемент 0 набора — первый операнд по коду книги.
				//
				// Книга зовёт 192 сто шестьдесят пять раз и **всегда** с
				// признаком 1 (счётчики 2, 3, 4 и 28).
				Value flag = pop();
				Value count = pop();
				uint elements = count.number & 0xff;
				if (4 + 4 * elements != cleanup) {
					debug(1, "ToolBook: 192 элементов %u, но cleanup %u @0x%x",
							elements, cleanup, ip - 4);
					return false;
				}
				Common::Array<Value> items;
				for (uint i = 0; i < elements; i++)
					items.push_back(pop());
				Value collection = newCollection();
				Common::Array<Value> &store = _stackStore[collection.collectionId];
				if (flag.number) {
					for (uint i = 0; i < items.size(); i++)
						stackPush(store, items[i]);
				} else {
					store.resize(elements);
					for (uint i = 0; i < items.size(); i++)
						store[elements - 1 - i] = items[i];
				}
				debug(2, "ToolBook: набор %u из %u элементов (признак %u)",
						collection.collectionId, store.size(), flag.number);
				for (uint i = 0; i < store.size(); i++)
					debug(3, "    набор %u[%u] = %s", collection.collectionId, i,
							describe(store[i]).c_str());
				if (wantsResult) {
					collection.width = resultWidth;
		...
		case 0x21: case 0x22: case 0x23: {
			...
			if (id == 8) {
				...
				// Переход на страницу **со зрительным эффектом**. Книга зовёт это
				// всегда одинаково: `411(строка эффекта, 12, ссылка на страницу, 0)`
				// — например `("vertical split out speed 2000", 12, zast4, 0)`.
				//
				// Обёртка RUN67:0x1104 сперва разбирает строку эффекта
				// (`MTB40RCR.65 AsymTbkFxParseString`), а потом по флагам решает,
				// куда идти: у 12 бит 0x40 не взведён, и ветка RUN67:0x1189
				// уходит на переход по третьему операнду. То есть цель перехода —
				// не догадка, она стоит в кадре отдельно от эффекта.
				//
				// **Отступление:** сам эффект (шторка, наплыв) не рисуем — страница
				// сменяется сразу. Это единственное, что теряется; куда идти,
				// известно точно (ledger/0084).
				Value extra = pop();
				Value target = pop();
				Value flags = pop();
				Value effect = pop();
				(void)extra;
				if (!target.isString && !target.isObject) {
					debug(1, "ToolBook: builtin 411 без страницы @0x%x", ip - 3);
					return false;
				}
				debug(2, "ToolBook: переход на «%s» эффектом «%s» (флаги %u, эффект не рисуется)",
						target.string.c_str(), valueString(effect).c_str(), flags.number);
				navigateTo(target.string);
				...
				// `sndPlaySound(путь, флаги)`. Обёртка RUN89:0x0000 не зовёт
				// функцию напрямую: она берёт из ресурсов строку 0x13ea, отдаёт её
				// в `LoadLibrary` (KERNEL.95), затем строку 0x13eb — в
				// `GetProcAddress` (KERNEL.50). Обе строки лежат рядом в
				// `MTB40UTL.DLL` (@0xef00): **`MMSYSTEM.DLL`** и
				// **`sndPlaySound`** — так что гадать про «какой-то звук» не
				// пришлось.
				//
				// Флаги считает пара `sbb ax,ax; neg ax` — та самая ловушка из
				// `CLAUDE.md`, читается наизнанку. `cmp [bp+6],1`: аргумент 1 даёт
				// 2 (`SND_NODEFAULT`, синхронно), аргумент 0 — 3
				// (`SND_ASYNC|SND_NODEFAULT`).
				//
				// **Отступление:** синхронный вариант мы всё равно играем
				// асинхронно. Ждать конца звука внутри обработчика значило бы
				// встать всей VM; книга на этом пути глушит звук `null.wav`, где
				// разницы нет. Если где-то понадобится настоящее ожидание — это
				// первое место, куда смотреть (ledger/0084).
				Value flags = pop();
				Value path = pop();
				bool played = playSoundFile(valueString(path), &_effectHandle);
				debug(2, "ToolBook: sndPlaySound «%s» %s -> %d",
						valueString(path).c_str(),
						flags.number == 1 ? "синхронно" : "асинхронно", played);
				pushNumber(played ? 1 : 0, op == 0x22 ? 2 : 4);
			} else if (id == 487) {
				// `TbkTimerStop` (RUN69:0x2e58 -> MTB40MM.7): `retf 2`, один
				// операнд — номер таймера. Таймеров движок не заводит (486,
				// `TbkTimerStartNew`, не реализован), поэтому останавливать нечего.
				//
				// Это не заглушка «лишь бы прошло»: страница меню зовёт 487 с нулём
				// в самом начале обработчика — снять таймер, которого может и не
				// быть, — и результат тут же выбрасывает (`adjustStack 4`).
				// Отвечать «остановлено» на «останови несуществующий» верно и для
				// среды (ledger/0084).
				Value timer = pop();
				debug(2, "ToolBook: таймер %u остановлен (таймеров нет)", timer.number);
				...
				if (selector.number == 0x4222) {
					// Это спрашивают не у окна, а у мультимедийного клипа: у 138
					// разбор идёт по классу объекта, и `0x4222..0x422b` — ветка
					// класса 0x64 (таблица переходов RUN66:0x0070), которая целиком
					// уходит в мультимедийную подсистему (seg124 → seg123, seg46,
					// `MTB40LNL`). Клипов движок не проигрывает вовсе, поэтому
					// честный ответ — «нельзя»: сама среда отвечает ложью ровно
					// тогда, когда клип не разрешается.
					//
					// У книги на этот случай есть своя ветка. Обработчик
					// `firstIdle` страницы `zast2` (книга @0x174c06) спрашивает это
					// у клипа «NMG» и по лжи уходит на страницу `zast2a`, минуя
					// проигрывание (builtin'ы 450/459/454). То есть заставка
					// пропускается так, как задумано для машины, которая её не
					// тянет, а не упирается в барьер (ledger/0084).
					//
					// Звука это не касается: он идёт мимо клипов, через
					// `mciSendString` (в логе — открытие `.\midi\toy.mid`).
					debug(2, "ToolBook: клип «%s» свойство 0x4222 -> нет (клипы не проигрываются)",
							object.string.c_str());
					pushNumber(0, op == 0x22 ? 2 : 4);
					break;
				}
				...
				// Свёртка по набору. Обёртка RUN87:0x0faa (`retf 6`: указатель
				// на приёмник и набор) зовёт общий обход RUN87:0x130a с кодом
				// действия 3. Обход берёт размер (`StackGetSize`), затем каждый
				// элемент (`StackElement`), приводит его к числу и складывает с
				// накопителем в RUN87:0x143a. Там же читаются все четыре кода:
				// 0 — среднее (сумма на выходе делится на число элементов,
				// 0x13e8), 1 — сумма, 2 — минимум (`fcom`+`jae`), 3 — максимум.
				// Обёртки под 0, 1 и 2 в среде есть, но в таблице встроенных
				// номеров этой сборки их нет — из книги достижим только 118.
				//
				// Не набор или пустой набор — у среды ошибка сценария
				// (`CDBSetPlErr` на 0x1349), поэтому движок останавливается.
				//
				// Приёмник — десятибайтовое место, отведённое опкодом `0x0f` и
				// адресуемое `0x59`, ровно как у арифметики (ledger/0082).
				Value destination = pop();
				Value collection = pop();
				const Common::Array<Value> *items = collectionOf(collection);
				if (!destination.isStackRef || destination.stackRef >= stack.size() ||
						!items || items->empty()) {
					debug(1, "ToolBook: builtin 118 — приёмник или набор не разобраны @0x%x",
							ip - 3);
					return false;
				}
				int32 outcome = numberOf((*items)[0]);
				for (uint i = 1; i < items->size(); i++)
					outcome = MAX(outcome, numberOf((*items)[i]));
				Value computed;
				computed.width = 10;
				computed.type = 0x22;
				computed.number = (uint32)outcome;
				stack[destination.stackRef] = computed;
				debug(2, "ToolBook: максимум набора %u из %u элементов -> %d",
						collection.collectionId, items->size(), outcome);
			} else if (id == 165) {
				// Дописывание в набор с заведением по месту. Обёртка
				// RUN67:0x0d90 (`retf 8`) проверяет второй операнд на
				// каноническую пустую ссылку (`[bp+0xa]==1 && [bp+0xc]==0x400`)
				// и в этом случае сама заводит пустой набор `ValueNewStack(0)`.
				// Дальше в любом случае `StackPush` (MTB40BAS.134) и возврат
				// набора.
				Value element = pop();
				Value target = pop();
				if (!target.collectionId) {
					if (!isNullValue(target)) {
						debug(1, "ToolBook: builtin 165 — приёмник не набор и не пустая ссылка @0x%x",
								ip - 3);
						return false;
					}
					target = newCollection();
				}
				Common::Array<Value> *store = collectionOf(target);
				if (!store) {
					debug(1, "ToolBook: builtin 165 — набор %u потерян @0x%x",
							target.collectionId, ip - 3);
					return false;
				}
				stackPush(*store, element);
				debug(2, "ToolBook: в набор %u дописано %s, стало %u",
						target.collectionId, describe(element).c_str(), store->size());
				target.width = 4;
				stack.push_back(target);
			} else if (id == 166) {
				// `StackPop`. Обёртка RUN67:0x0de8 (`retf 4`) на пустой ссылке
				// сразу ругается (`CDBSetPlErr` @0x0e19), а иначе зовёт
				// MTB40BAS.136. Снимается элемент 0 — у среды это физический
				// хвост, тот самый, что дописал `StackPushOne`.
				//
				// Пустой набор среда тоже считает ошибкой, но результат при этом
				// возвращает — то самое «пусто», которым обёртка завела место
				// (0, 0x400). Движок так и делает, только шумит в лог.
				Value collection = pop();
				Common::Array<Value> *store = collectionOf(collection);
				if (!store) {
					debug(1, "ToolBook: builtin 166 — операнд не набор @0x%x", ip - 3);
					return false;
				}
				Value element;
				if (store->empty())
					debug(1, "ToolBook: builtin 166 — набор %u пуст @0x%x",
							collection.collectionId, ip - 3);
				else {
					element = (*store)[0];
					store->remove_at(0);
				}
				debug(2, "ToolBook: из набора %u снято %s, осталось %u",
						collection.collectionId, describe(element).c_str(), store->size());
				element.width = 4;
				stack.push_back(element);
			} else if (id == 214) {
				// «Элемент N набора», нумерация с единицы. Обёртка RUN86:0x0000
				// (`retf 6`: слово-номер и набор) берёт размер только если тег
				// операнда — набор (`and al,0xfc; cmp 0x40`), проверяет
				// `1 ≤ N ≤ размер` и зовёт `StackElement` с `N − 1`. Вне
				// диапазона — каноническая пустая ссылка (0x0d5c: `1`, `0x400`).
				Value index = pop();
				Value collection = pop();
				const Common::Array<Value> *items = collectionOf(collection);
				uint32 at = index.number;
				Value element;
				element.number = 0x04000001;
				if (items && at >= 1 && at <= items->size())
					element = (*items)[at - 1];
				else
					debug(2, "ToolBook: набор %u не отдал элемент %u -> пусто",
							collection.collectionId, at);
				debug(2, "ToolBook: набор %u[%u] -> %s", collection.collectionId, at,
						describe(element).c_str());
				element.width = 4;
				stack.push_back(element);
			} else {
...
void ToolBookEngine::playMedia(const Common::String &alias) {
	...
	stopMedia(alias);
	if (playSoundFile(_mciAliases[alias], &_mediaHandle))
		_playingAlias = alias;
}

bool ToolBookEngine::playSoundFile(const Common::String &bookPath, Audio::SoundHandle *handle) {
	// Путь книги написан по-досовски — с обратными косыми и, возможно, с буквой
	// диска. Дерево игры у нас плоское по каталогам установки, поэтому приводим
	// путь к тому виду, в котором его найдёт SearchMan (ledger/0070).
	Common::String path = bookPath;
	...
	if (lowered.hasSuffix(".wav")) {
		...
		if (sound) {
			...
			_mixer->playStream(Audio::Mixer::kSFXSoundType, handle, sound);
